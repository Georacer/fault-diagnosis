...
%                 {'V_i ni u ni v ni w sub kinematics expr -V_i+sqrt(u*u+v*v+w*w)'};...
                %% The follwing equations are causing me trouble in analytical solutions. I am removing them
%                 {'chi ni u ni v ni w ni Phi ni Theta ni Psi sub kinematics expr -chi+atan2((cos(Theta)*sin(Psi))*u+(cos(Phi)*cos(Psi)+sin(Phi)*sin(Theta)*sin(Psi))*v+(-sin(Phi)*cos(Psi)+cos(Phi)*sin(Theta)*sin(Psi))*w,(cos(Theta)*cos(Psi))*u+(-cos(Phi)*sin(Psi)+sin(Phi)*sin(Theta)*cos(Psi))*v+(sin(Phi)*sin(Psi)+cos(Phi)*sin(Theta)*cos(Psi))*w)'};... % ni needed here to keep the system semi-explicit DAE
%                 {'Gamma ni u ni v ni w ni Phi ni Theta ni Psi ni V_i sub kinematics expr -Gamma+asin(-(-sin(Theta))*u+(sin(Phi)*cos(Theta))*v+(cos(Phi)*cos(Theta))*w/V_i)'};...
                %%
%                 {'V_g V_i Gamma sub kinematics expr -V_g+V_i*cos(Gamma)'};... % Is a "ni" in front of V_i reasonable? - I choose to let it as is
            ...
            kin = [kin;...
                ...
                {'nos Alpha w_r u_r sub airdata expr -Alpha+atan2(w_r,u_r)'};...
...
%                 {'T_atot_z T_a_z sub aerodynamics'};...
                ...
                {'F_D ni q_bar C_D par S sub aerodynamics expr -F_D+q_bar*S*C_D'};...
                {'F_Y ni q_bar C_Y par S sub aerodynamics expr -F_Y+q_bar*S*C_Y'};...
                {'F_L ni q_bar C_L par S sub aerodynamics expr -F_L+q_bar*S*C_L'};...
                {'C_D ni V_a ni Alpha ni q ni ni delta_e par C_D_0 par C_D_Alpha par C_D_q par C_D_delta_e par c sub aerodynamics expr -C_D+C_D_0+C_D_Alpha*Alpha+C_D_q*c*0.5/V_a*q+C_D_delta_e*delta_e'};...
                {'C_Y ni V_a ni Beta ni p ni r ni delta_r par C_Y_0 par C_Y_Beta par C_Y_p par C_Y_delta_r par b sub aerodynamics expr -C_Y+C_Y_0+C_Y_Beta*Beta+C_Y_p*b*0.5/V_a*p+C_Y_delta_r*delta_r'};...
                {'C_L ni V_a ni Alpha ni ni q ni delta_e par C_L_0 par C_L_Alpha par C_L_q par C_L_delta_e par c sub aerodynamics expr -C_L+C_L_0+C_L_Alpha*Alpha+C_L_q*c*0.5/V_a*q+C_L_delta_e*delta_e'};...
            ...
            % Propeller
            dyn = [dyn;...
                {'F_t_x P_prop ni n_coeff ni u_r sub propeller expr -F_t_x+P_prop*abs(n_coeff/u_r)'};... % Using propeller model
                ...
                {'P_prop C_p ni rho ni n_prop param D sub propeller expr -P_prop+C_p*rho*n_prop^3*D^5'};...
...
%                 {'h ni T_0 ni P ni P_0 h_0 par L_0 par g_0 par M_0 par R_star sub athmosphere expr -h+T_0/L_0*(pow(P/P_0,g_0*M_0/R_star/L_0)-1)+h_0'};... % Barometric altitude calcuation
                ...
                {'u_w sub atmoshpere expr -u_w+0'};...
                {'v_w sub atmoshpere expr -v_w+0'};...
                {'w_w sub atmoshpere expr -w_w+0'};...
                % RR 1
...
%                 {'w_e sub atmosphere expr -w_e+0'};... % TODO Verify
            ...
            msr = [msr;...
                ...
                {'msr Phi_m Phi sub sensors expr -Phi+Phi_m'};... % Euler angles
                {'msr Theta_m Theta sub sensors expr -Theta+Theta_m'};...
                {'msr Psi_m Psi sub sensors expr -Psi+Psi_m'};...
...
%                 {'msr chi_gps chi sub sensors expr -chi+chi_gps'};...
                ...
                {'msr z_0_gps z_0 sub initializations expr -z_0+z_0_gps'};... % Initialization altitude
                % RR2
%                 {'z_0 h_0 par r_0 sub initializations expr -z_0+r_0*h_0/(r_0-h_0)'};...
                {'fault msr P_bar P sub sensors expr -P+P_bar'};... % Barometer reading
                {'fault msr T_m T sub sensors expr -T_m+T'};... % Therometer reading
