...
%                 {'dot_h ni u ni Theta ni v ni Phi ni w expr dot_h-u*sin(Theta)+(v*sin(Phi)+w*cos(Phi))*cos(Theta)'};...
            ...
            der = [...
                ...
                {'int dot_p dot msr p expr differentiator'};...
                {'int dot_q dot msr q expr differentiator'};...
                {'int dot_r dot msr r expr differentiator'};...
                {'int dot_Phi dot msr Phi expr differentiator'};...
                {'int dot_Theta dot msr Theta expr differentiator'};...
%                 {'int dot_Psi dot Psi expr differentiator'};...
%                 {'int dot_h dot h expr differentiator'};...
                ];
            
            dyn = [...
                {'Xa ni V a de ni msr qbar par CX0 par CXa par CXde S expr -Xa+(CX0+CXa*a+CXde*de)*qbar*S'};...
                {'Ya ni V ni b ni p ni r ni msr da ni msr dr ni qbar par CY0 par CYb par CYp par CYr par CYda par CYdr par bw par S expr -Ya+(CY0+CYb*b+CYp*p*bw/2/V+CYr*r*bw/2/V+CYda*da+CYdr*dr)*qbar*S'};...
                ...
                {'L ni V ni b ni p ni r ni da ni dr ni qbar par Cl0 par Clb par Clp par Clr par Clda par Cldr par S par bw expr -La+(Cl0+Clb*b+Clp*p*bw/2/V*Clr*r*bw/2/V+Clda*da+Cldr*dr)*qbar*S*b'};...
                {'M ni V ni a ni q ni de ni qbar par Cm0 par Cma par Cmq par Cmde par S par c expr -Ma+(Cm0+Cma*a+Cmq*q*c/2/V+Cmde*de)*qbar*S*c'};...
                {'N ni V ni b ni p ni r ni da ni dr ni qbar par Cn0 par Cnb par Cnp par Cnr par Cnda par Cndr par S par bw expr -Na+(Cn0+Cnb*b*Cnp*p*bw/2/V+Cnr*r*bw/2/V+Cnda*da+Cndr*dr)*qbar*S*b'};...
            ...
            msr = [...
                {' Xt inp Xt_c expr -Xt+Xt_c'};...
%                 {' da inp da_c expr -da+da_c'};...
                {'fault de inp de_c expr -de+de_c'};...
%                 {' dr inp dr_c expr -dr+dr_c'};...
                {'fault V msr V_m expr -V+V_m'};...
                {' a msr a_m expr -a+a_m'};...
                {' b msr b_m expr -b+b_m'};...
...
%                 {' h msr h_m expr -h+h_m'};...
%                 {' qbar msr qbar_m expr -qbar+qbar_m'};...
                ];
