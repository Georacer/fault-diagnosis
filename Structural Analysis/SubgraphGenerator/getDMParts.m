...
%
% Duplicated and edite by George Zogopoulos-Papaliakos to override the
% default function functionality
...
%     foo.col = sort(q(s(idx):s(idx+1)-1));
    foo.row = p(r(idx):r(idx+1)-1);
    foo.col = q(s(idx):s(idx+1)-1);    
    M0{end+1} = foo;
    idx = idx+1;
  end
...
%     Mp.col = sort(q(s(idx):s(idx+1)-1));
    Mp.row = p(r(idx):r(idx+1)-1);
    Mp.col = q(s(idx):s(idx+1)-1);
  else
    Mp.row = [];
    Mp.col = [];
  end
