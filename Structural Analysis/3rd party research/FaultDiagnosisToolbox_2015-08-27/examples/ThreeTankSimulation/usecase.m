...
% sm.BipartiteToLaTeX('bipart.tex','fault', true,'shortnames',false,...
...
Gamma1 = sm.Matching(m01); % coompute matching
sm.SeqResGen( Gamma1, redeq1,'ResGen1', 'implementation', 'discrete' );
...
Gamma2 = sm.Matching(m02); % compute matching
sm.SeqResGen( Gamma2, redeq2,'ResGen2', 'implementation', 'discrete' );
...
Gamma3 = sm.Matching(m03);
sm.SeqResGen( Gamma3, redeq3,'ResGen3', 'implementation', 'discrete' );

% Observer based residual generator based on the same equations as Resgen1
eqr4 = mso2;
linpoint.x0 = [0,0,0,0,0,0];
linpoint.z0 = [0;0;0];
[A4,C4] = sm.ObserverResGen( eqr4, 'ResGen4', 'linpoint', linpoint, 'parameters', params );
...
FSM = sm.FSM({mso2, mso2, mso1, mso2});
figure(30)
spy(FSM,30)
set(gca, 'YTick', 1:4, 'XTick', 1:6,...
  'YTickLabel', {'r1', 'r2', 'r3', 'r4'},'XTickLabel',sm.f,...
  'box', 'off');
xlabel('Fault')
ylabel('Residual')
...
meas = [1,6,4]; % p1, q2, q0
M = zeros(size(sim{1}.z0,2),length(meas));
M(meas,1:length(meas)) = eye(length(meas));
for k=1:length(sim)
  sim{k}.z = sim{k}.z0*M + randn(size(sim{k}.z0,1),length(meas))*diag(nstd(meas));
end
...
N = length(t);
r1 = zeros(N,length(sim)); % FSM: 1,4,5
r2 = zeros(N,length(sim)); % FSM: 1,4,5
r3 = zeros(N,length(sim)); % FSM: 2,3,4,5,6
r4 = zeros(N,length(sim)); % FSM: 2,3,4,5,6

M4 = [eye(2) zeros(2,4);zeros(4,6)];
for fi=1:length(sim)
  % Simulate residual generators implemented in continuous time    
  x0 = [sim{fi}.z0(1,1:5)';0]; % p1,p2,q0,q1,q2,r
  ...
  state1.p1 = sim{fi}.z(1,1); state1.p2 = sim{fi}.z(1,2);
  state2.p1 = sim{fi}.z(1,1); state2.p2 = sim{fi}.z(1,2);
  state3.p1 = sim{fi}.z(1,1); state3.p2 = sim{fi}.z(1,2); state3.p3 = sim{fi}.z(1,3);
  
  for k=1:N
    [r1(k,fi), state1] = ResGen1( sim{fi}.z(k,:), state1, params, 1/fs );
    [r2(k,fi), state2] = ResGen2( sim{fi}.z(k,:), state2, params, 1/fs );
    [r3(k,fi), state3] = ResGen3( sim{fi}.z(k,:), state3, params, 1/fs );
  end
end
r1(1:2,:) = 0;
r3(1,:) = 0;
