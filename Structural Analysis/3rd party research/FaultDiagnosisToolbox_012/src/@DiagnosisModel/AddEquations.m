function sm = AddEquations( model, eqs, xvars, zvars, fvars, params, varargin )
% ADDEQUATIONS  Add new equations to a given model
%
%  model2 = model.AddEquations( eqs, xvars, zvars, fvars, params, options )
%
...
%         model.AddEquations( eqs, xvars, zvars, fvars, params);
%
%      To create a new object, with the new equations, without
%      modifying the original model, instead write
%         model2 = model.AddEquations( eqs, xvars, zvars, fvars, params);
...
%  http://opensource.org/licenses/MIT)
  ...
  for kk=1:numel(eqs)  
    ...
    sm.X(end+1,:) = zeros(1,nx+nxnew); sm.X(end,xIdx>0) = 1;

    if nfnew>0
      sm.F = [sm.F zeros(ne,nfnew)];
    end
    sm.F(end+1,:) = zeros(1,nf+nfnew); sm.F(end,fIdx>0) = 1;

    if nznew>0
      sm.Z = [sm.Z zeros(ne,nznew)];
    end
    sm.Z(end+1,:) = zeros(1,nz+nznew); sm.Z(end,zIdx>0) = 1;
