classdef VariableIdGen < handle
  ...
  methods
    ...
    function [r,rl]=NewX(m)
      r = sprintf('%s%d',m.xlabel, m.x);
      rl = sprintf('%s_{%d}',m.xlabel, m.x);
      m.x = m.x + 1;
    end
    function [r,rl]=NewZ(m)
      r = sprintf('%s%d',m.zlabel, m.z);
      rl = sprintf('%s_{%d}',m.zlabel, m.z);
      m.z = m.z + 1;
    end
    function [r,rl]=NewF(m)
      r = sprintf('%s%d',m.flabel, m.f);
      rl = sprintf('%s_{%d}',m.flabel, m.f);
      m.f = m.f + 1;
    end
    function [r,rl]=NewE(m)
      r = sprintf('%s%d',m.elabel, m.e);
      rl = sprintf('%s_{%d}',m.elabel, m.e);
      m.e = m.e + 1;
    end
  end
end